...
{
    painter->save();
    QLinearGradient batteryGradient(QPointF(0, 0), QPointF(0, width()));
    ...
    int margin = qMin(width(), height()) / 25;
    double unit = (batteryRect.height() - (margin * 2)) / 100;
    double hight = currentValue * unit;
    QPointF topLeft(batteryRect.bottomLeft().x() + margin, batteryRect.bottomLeft().y() - hight - margin);
...
void Battery::drawVal(QPainter *painter)
{
    painter->save();
    QFontMetrics textFont(font());
    QString powerStr = QString::asprintf("%d%%", int(currentValue));
